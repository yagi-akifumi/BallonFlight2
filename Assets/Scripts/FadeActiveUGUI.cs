...
public class FadeActiveUGUI : MonoBehaviour
{
    ...
    [Header("上昇量")] public float moveTime = 1.0f;
    ...
    void Update()
    {
        ...
        if (trigger.isOn)
        {
            ...
            {
                Debug.Log("test");
                cg.alpha = timer / moveTime;//アルファ値を1になるまで上昇させる
                cg.transform.position += Vector3.up * (moveDis / moveTime) * speed * Time.deltaTime;//速さを調節
